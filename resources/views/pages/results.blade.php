...
@section('title', 'Turnierergebnisse')

@section('content')

<p>Auf dieser Seite findest du Ergebnisse vergangener Turniere.</p>
...
<table class="data-table striped">
    ...
    @foreach ($tournaments as $row)
        <tr data-date="{{ $row->date }}" data-type="{{ $row->type }}" data-city="{{ $row->store->city }}">
            ...
            <td><a href="{{ route('tournament', [ 'id' => $row->id ]) }}">{{ date_only($row->date) }}</a></td>
